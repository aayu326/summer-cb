...
// ==============================================
const otpStore = {}; // { email: { otp, name, phone, expiresAt } }
...
app.post('/api/send-otp', async (req, res) => {
  try {
    ...
    otpStore[email] = { otp, name, phone, expiresAt: Date.now() + 10 * 60 * 1000 };

    const otpMailOptions = {
      from: EMAIL_CONFIG.auth.user,
      to: email,
      subject: '🔐 Your OTP — Sumeru Infrastructures Chatbot',
      html: `
        <!DOCTYPE html><html><head><meta charset="UTF-8">
        <style>
          body{font-family:'Segoe UI',Arial,sans-serif;background:#f0f4f8;margin:0;padding:0;}
          .wrapper{max-width:480px;margin:30px auto;border-radius:16px;overflow:hidden;box-shadow:0 10px 40px rgba(0,0,0,0.12);}
          .header{background:linear-gradient(135deg,#1a4a2e 0%,#0d2d1a 100%);padding:35px 30px;text-align:center;}
          .header h1{color:white;font-size:22px;margin-bottom:6px;}
          .header p{color:rgba(255,255,255,0.7);font-size:13px;}
          .body{background:white;padding:35px 30px;text-align:center;}
          .otp-box{background:linear-gradient(135deg,#f0f7f3,#e8f4ee);border:2px dashed #1a4a2e;border-radius:16px;padding:28px;margin:24px 0;}
          .otp-label{font-size:13px;color:#666;margin-bottom:12px;font-weight:600;text-transform:uppercase;letter-spacing:1px;}
          .otp-code{font-size:42px;font-weight:900;color:#1a4a2e;letter-spacing:10px;}
          .validity{font-size:13px;color:#888;margin-top:12px;}
          .note{background:#fff8f0;border:1px solid rgba(196,127,42,0.3);border-radius:10px;padding:14px;font-size:13px;color:#666;margin-top:20px;}
          .footer{background:#1a4a2e;padding:20px;text-align:center;}
          .footer p{color:rgba(255,255,255,0.6);font-size:12px;}
        </style></head>
        <body>
          <div class="wrapper">
            <div class="header"><h1>🏢 Sumeru Infrastructures</h1><p>Email Verification Code</p></div>
            <div class="body">
              <p style="font-size:16px;color:#333;">Hello <strong>${name}</strong>! 👋</p>
              <p style="color:#555;font-size:14px;margin-top:8px;">Use the OTP below to verify your email and start chatting.</p>
              <div class="otp-box">
                <div class="otp-label">Your One-Time Password</div>
                <div class="otp-code">${otp}</div>
                <div class="validity">⏱️ Valid for <strong>10 minutes</strong></div>
              </div>
              <div class="note">🔒 Do not share this OTP with anyone.</div>
            </div>
            <div class="footer"><p>Sumeru Infrastructures — Dehradun, Uttarakhand</p></div>
          </div>
        </body></html>
      `
    };
...
app.post('/api/verify-otp', async (req, res) => {
  try {
    ...
    console.log('✅ OTP verified & user registered:', { name, email, phone });
    const emailSent = await sendAdminEmail({ name, email, phone });

    res.json({ success: true, message: 'Email verified! Welcome to Sumeru Infrastructures.', emailSent });
